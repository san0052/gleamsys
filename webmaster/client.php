...
<td vAlign=top align="center" width="99%"><!-- Start Body Here -->
  <table width="100%" border="0" align="center" cellpadding="2" cellspacing="0">
	  ...
	  <? //show all record
	   ...
	   ?>
	    ...
	    <table width="98%" align="center" cellpadding="6" cellspacing="1" class="tborder_new">
          <thead>
            <tr>
              <td colspan="11" align="left">&nbsp;<span class="style2">Manage Project Information</span> </td>
            </tr>
          </thead>
          <tbody>
            <? if($_REQUEST['m']){ ?>
		...
		<?php
		 	$sql    = "SELECT * FROM ".$cfg['DB_CLIENT_INFO']." WHERE 
		...
		?>
            <tr class="<?=($i%2==0)?'row1':'row2'?>">
				...
				<td align="center" colspan="2"><?=$row['projectName']?></td> 
				...
				<td align="center" colspan="3" >&nbsp;<?=$row['projectCountry'];?></td>		 	  
				  ...
				  <a href="client-process.php?act=<?=($row['status']=='A')?'Inactive':'Active'?>&pageno=<?=($_REQUEST['pageno']!="")?$_REQUEST['pageno']:'0'?>&id=<?=$row['id']?>" class="<?=($row['status']=='A')?'greenbuttonelementsNew':'redbuttonelementsNew'?>"><?=($row['status']=='A')?'Active':'Inactive'?></a>
			  ...
			  <td align="center">
				  ...
				  <a href="client-process.php?act=edit&pageno=<?=($_REQUEST['pageno']!="")?$_REQUEST['pageno']:'0'?>&id=<?=$row['id']?>"><img src="images/edit.gif" title="Edit" width="16" height="16" border="0" /></a>
			  
				  <a href="client-process.php?act=del&pageno=<?=($_REQUEST['pageno']!="")?$_REQUEST['pageno']:'0'?>&id=<?=$row['id']?>"><img src="images/drop.gif" title="Delete" width="16" height="16" border="0" onClick="return confirm('Do you really want to delete this record ?');" /></a>
		...
		<div class="bottomsecc">		
			<a class="brownbttn" href="client-process.php?act=add&pageno=<?=($_REQUEST['pageno']!="")?$_REQUEST['pageno']:'0'?>">Add Project Info</a>
		</div>
	...
	{ 
	?>
		<form name="frmadd" id="frm2" method="post" action="client-process.php" onsubmit="return add_typ_value()" enctype="multipart/form-data"><p>
          ...
          <table width="98%" align="center" cellPadding=6 cellSpacing=1 class="tborder_new"> 
            <thead> 
              <tr> 
                <td colspan="5" align="left" class="style2">&nbsp;Add Poject Info</td> 
              </tr> 
            </thead> 
            <tbody> 
              <?php if($_REQUEST['m']){ ?>
			  ...
			  <? } ?>
			<tr class="row1"> 
                <td width="30%" align="left" class="leftBarText"><span class="leftBarText_new">Project Name</span> <span class="redstar">*</span></td> 
                <td width="70%" colspan="4" align="left"><input name="projectName" type="text" class="forminputelement" id="bannerTitle" value=""/>&nbsp;&nbsp;</td>
			</tr>

			<tr class="row1"> 
                <td width="30%" align="left" class="leftBarText"><span class="leftBarText_new">Project Image</span> <span class="redstar">*</span></td> 
	...
	// show edit window
	  ...
	  ?>
	  <form name="frmedit"  id="frm3" method="post" action="client-process.php" enctype='multipart/form-data' onsubmit="return edit_typ_value()">
          <p>
          ...
          <table width="98%" align="center" cellPadding=6 cellSpacing=1 class="tborder_new"> 
            ...
            <tbody> 
			  ...
			  <tr class="row2"> 
	                <td width="30%" align="left" class="leftBarText"><span class="leftBarText_new">Project Name</span></td> 
	                <td  width="70%" colspan="4" class="leftBarText" align="left" valign="top">
	                	<input type="text" name="projectName" id="projectName" value="<?= $row['projectName'];?>" style="width:80%"></td>
		  		</tr>

              <tr class="row1"> 
                <td width="30%" align="left" class="leftBarText"><span class="leftBarText_new">Brand Logo</span> <span class="redstar">*</span></td> 
			...
			<tr class="row2"> 
	            ...
	            <td  width="70%" colspan="4" class="leftBarText" align="left" valign="top"><input type="text" name="projectCountry" id="projectCountry" value="<?= $row['projectCountry'];?>"></td>
		  	</tr>
