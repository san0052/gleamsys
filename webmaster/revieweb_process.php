...
//show all record
case'view':
	$heart->redirect('revieweb.php?show=view&id='.$_REQUEST['id'].'&pageno='.$_REQUEST['pageno']);
exit();
break;
// show edit window
case'edit':
	$heart->redirect('revieweb.php?show=edit&id='.$_REQUEST['id'].'&pageno='.$_REQUEST['pageno']);
exit();
break;
//delete record
case 'del':
	
     $sql="DELETE FROM" .$cfg['DB_PRODUCT_REVIEWS']. "WHERE `id` IN (".$_REQUEST['id'].")";
	 $heart->sql_query($sql);
	 
	$heart->redirect('revieweb.php?m=3&pageno='.$_REQUEST['pageno']);
break; 
case 'insert':
...
$sql="INSERT INTO ".$cfg['DB_PRODUCT_REVIEWS']." SET 
	...
	$heart->redirect('revieweb.php?category='.$_REQUEST['secpid'].'&m=1');
	$heart->redirect('revieweb.php?&m=1');
break;
// update record

//show add window
case'add':
	$heart->redirect('revieweb.php?show=add&pageno='.$_REQUEST['pageno']);	
break;
//change status

case'Active':
  $sql="UPDATE ".$cfg['DB_PRODUCT_REVIEWS']."
	 SET 
	`status` = 'A'  WHERE `id` IN (".$_REQUEST['id'].")";
	 $heart->sql_query($sql);
 $heart->redirect('revieweb.php?m=2&pageno='.$_REQUEST['pageno']);
...
case 'update':
	...
	$state          = addslashes($_REQUEST['state']);
		...
		$sql="UPDATE ".$cfg['DB_PRODUCT_REVIEWS']."
      	...
      	$heart->redirect('revieweb.php?&m=2&pageno='.$pageno);
...
$sql="INSERT INTO ".$cfg['DB_PRODUCT_REVIEWS']." SET `pd_name` = '".$pname."',`pd_price` = '".$price."',	`strike_price` = '".$sprice."',	`pd_description` = '".$desc."',`category`='".$cat."',`disclaimer` = '".$prod_dis."',`notes` = '".$prod_note."',`location` = '".$prod_loc."',`pd_date`=NOW(),`pd_featured`='".$pf."',`pd_rightbar`='".$pr."',`pd_bestseller`='".$bp."',`status`='A',`siteId`= '".$cfg['SESSION_SITE']."' " ;	
		$heart->sql_query($sql);
	 ...
	 //insert category in the another table
		...
		{
			$sqlorders="INSERT INTO ".$cfg['DB_PRODUCT_REVIEWS_CAT']." SET `pd_id` = ".$last_id." ,`siteId`= '".$cfg['SESSION_SITE']."',`cat_id` =".$val.",`status`='A' ";
			$heart->sql_query($sqlorders);
		}
	 
	 //insert category in the another table
	 
	 $sqlorders="UPDATE ".$cfg['DB_PRODUCT_REVIEWS']." SET `order` = ".$last_id.",`isAddon`='Y' WHERE `pd_id` =".$last_id." AND `siteId`= '".$cfg['SESSION_SITE']."' ";
	 $heart->sql_query($sqlorders);
			...
			if($ph!="")
			{
				...
				$path="../".$cfg['PRODUCT_IMAGES'].''.$value;
					...
					chmod($path,0777);
					 ...
					 $sqlup="UPDATE ".$cfg['DB_PRODUCT_REVIEWS']." SET `pd_image`='".$value."',`pd_code`='".$code."' WHERE `pd_id`=".$last_id." AND `siteId`= '".$cfg['SESSION_SITE']."' ";
...
$cate_id=$_REQUEST['cate_id'];
	...
	$bp=addslashes(($_REQUEST['bp']=="1"))?'Y':'N';		
		 ...
		 $sql="UPDATE ".$cfg['DB_PRODUCT_REVIEWS']."
			...
			$sqld="DELETE FROM ".$cfg['DB_PRODUCT_REVIEWS_CAT']." WHERE `pd_id` = ".$last_id." ";
	 	...
	 	foreach($cate_id as $key=>$val)
		{
			$sqlorders="INSERT INTO ".$cfg['DB_PRODUCT_REVIEWS_CAT']." SET `pd_id` = ".$last_id." ,`cat_id` =".$val.",`status`='A' ";
			$heart->sql_query($sqlorders);
		}
		...
		foreach($_FILES['image_add']['name'] as $key=>$val)
		{
			...
			if($val!="")
			{
					...
					chmod($path,0777);
					 ...
					 $sqlup="UPDATE ".$cfg['DB_PRODUCT_REVIEWS']." SET `pd_image`='".$value."' WHERE `pd_id`=".$last_id." AND `siteId`= '".$cfg['SESSION_SITE']."' ";
					 $heart->sql_query($sqlup);
			}
			
		}
		$sqlnew1="UPDATE ".$cfg['DB_PRODUCT_REVIEWS']." SET `new_status` = 'n'";
		$heart->sql_query($sqlnew1); 
		$sqlnew2="UPDATE ".$cfg['DB_PRODUCT_REVIEWS']." SET `new_status` = 'y' WHERE `status`='A' ORDER BY `pd_id` DESC LIMIT 10";
...
case 'del_addon':
        $sql1="SELECT * FROM ".$cfg['DB_PRODUCT_REVIEWS']." WHERE  `pd_id` IN (".$_REQUEST['id'].")";
...
$sql="UPDATE " .$cfg['DB_PRODUCT_REVIEWS']. " SET `status`='D' WHERE `pd_id` IN (".$_REQUEST['id'].")";
	 $heart->sql_query($sql);
...
case'order':
	 ...
	 $num=is_numeric($value);
			 if($num==1)
			 {
			 $sql_menu="UPDATE ".$cfg['DB_PRODUCT_REVIEWS']." SET `order` = ".$value."  WHERE `pd_id` = ".$key." ";	
			 $heart->sql_query($sql_menu);	 
			 }
	 }
     $heart->redirect('revieweb.php?m=4'.$gourl.'&pageno='.$_REQUEST['pageno']);
break;
...
$maxrow=$heart->sql_numrows($res_cat);
?>
<select name="secpid" class="forminputelement" id="secpid" onchange="window.location.href='revieweb.php?category='+this.value">
<option value="0" selected="selected">Select Category</option>
<? 
 if($maxrow >0)
{
...
$heart->sql_query($sqlnew1); 
 $sqlnew2="UPDATE ".$cfg['DB_PRODUCT_REVIEWS']." SET `new_status` = 'y' WHERE `status`='A' ORDER BY `pd_id` DESC LIMIT 10";
$heart->sql_query($sqlnew2); 
 $heart->redirect('allrevieweb.php?m=2&pageno='.$_REQUEST['pageno']);
