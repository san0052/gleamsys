...
function category_availability_edit_new(id,pagename)
{
	...
	var url='category_availability.php?act='+pagename+'&str='+chkstr;
	http.open('get',url);
	http.onreadystatechange = handleResponsecatnameeditnew;
	http.send(null);
	}
	else{
	document.getElementById('exsists_edit').style.display = 'none';
...
function handleResponsecatnameeditnew()
{
	if(http.readyState == 4 && http.status == 200)
	{
...
else if(document.getElementById("prod_loc").value==''){
	...
	if(document.frm1.dropdown1.value=='delete')
		...
		if(flag > 0)
		 ...
		 if(confirm('Do you want to delete these records')==true)
		   {   
		   var pageno1=pageno;
		   window.location.href="city_process.php?act=muldel&id="+ar+"&pageno="+pageno1;
			 return true;
	       }
		   else
		   {
		...
		if(flag > 0)
		 ...
		 if(confirm('Do you want to activate these records')==true)
		   {   
		   var pageno1=pageno;
		    window.location.href="city_process.php?act=mulactive&id="+ar+"&pageno="+pageno1;
			 return true;
	       }
		   else
		   {
		...
		if(flag > 0)
		 ...
		 if(confirm('Do you want to inactivate these records')==true)
		   {   
		 var pageno1=pageno;
		window.location.href="city_process.php?act=mulinactive&id="+ar+"&pageno="+pageno1;
			 return true;
	       }
		   else
		   {
