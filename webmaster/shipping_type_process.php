...
//show all record
case'view':
	$heart->redirect('shipping_type.php?show=view&id='.$_REQUEST['id'].'&pageno='.$_REQUEST['pageno']);
exit();
break;
// show edit window
case'edit':
	$heart->redirect('shipping_type.php?show=edit&id='.$_REQUEST['id'].'&pageno='.$_REQUEST['pageno']);
exit();
break;
//delete record
case 'del':
...
case'Active':
  $sql="UPDATE ".$cfg['DB_SHIPPING_TYPE']."
	...
	`status` = 'A'  WHERE `id` IN (".$_REQUEST['id'].")";
	 $heart->sql_query($sql);
 $heart->redirect('shipping_type.php?m=2&pageno='.$_REQUEST['pageno']);
...
case 'update':
	...
	$amount         = addslashes($_REQUEST['amount']);
		...
		$sql="UPDATE ".$cfg['DB_SHIPPING_TYPE']."
      	...
      	$heart->redirect('shipping_type.php?&m=2&pageno='.$pageno);
