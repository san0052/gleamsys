...
page_header($cfg['ADMIN_TITLE']." - Key Category Management");
...
<script language="javascript" src="scripts/category_key.js"></script>
  ...
  <table width="100%" border="0" align="center" cellpadding="2" cellspacing="0">
    ...
    <tr>
	  ...
	  /* $returnCatId=($_REQUEST['catId']=="")?'0':$_REQUEST['catId'];*/
	   ...
	   ?>
              <form name="frm1" id="frm1" action="category_process_key.php" method="post">
                <input type="hidden" name="act" id="act" value="order" />
                <? $pageno=($_REQUEST['pageno']!='')?$_REQUEST['pageno']:'0'; ?>
                <input type="hidden" name="pageno" id="pageno" value="<?=$pageno?>" />
                <? if($parentId!=0){?>
                <div style="text-align:left; width:90%; padding-bottom:2px;"><a href="category_key.php?pId=0&pageno=<?=($_REQUEST['pageno']!="")?$_REQUEST['pageno']:'0'?>" class="linkcat">Parent</a>
                  <? if($_REQUEST['secpid']!=0){?>
                  &raquo; <a href="category_key.php?pId=<?=$_REQUEST['secpid']?>&pageno=<?=($_REQUEST['pageno']!="")?$_REQUEST['pageno']:'0'?>" class="linkcat">
                ...
                <table width="98%" align="center" cellpadding="6" cellspacing="1" class="tborder_new">
                  <thead>
                    <tr>
                      <td colspan="11" align="left">&nbsp;<span class="style2">Key Category Section</span> </td>
                    </tr>
                  </thead>
                  <tbody>
                    <? if($_REQUEST['m']){ ?>
		...
		$sql="SELECT * FROM ".$cfg['DB_KEYWORD_CATEGORY']."  WHERE `cat_parent_id`=".$parentId." AND `siteId`='".$cfg['SESSION_SITE']."' ORDER BY `order`";
			...
			?>
                    <tr class="<?=($i%2==0)?'row1':'row2'?>">
                      ...
                      <td align="left" colspan="3" >&nbsp;
                        <? if(count(getChildkeyCategories($row['id']))>0){?>
                        <a href="category_key.php?pId=<?=$row['id']?>&secpid=<?=$_REQUEST['pId']?>&pageno=<?=($_REQUEST['pageno']!="")?$_REQUEST['pageno']:'0'?>">
                        <?=$row['name']?>
                        </a>
                        <? }else{?>
                        <?=$row['name']?>
                      ...
                      <td align="center"><a href="category_process_key.php?act=<?=($row['status']=='A')?'Inactive':'Active'?>&pageno=<?=($_REQUEST['pageno']!="")?$_REQUEST['pageno']:'0'?>&id=<?=$row['id']?>&secpid=<?=($_REQUEST['secpid']!="")?$_REQUEST['secpid']:'0'?>&pId=<?=($_REQUEST['pId']!="")?$_REQUEST['pId']:'0'?>" class="<?=($row['status']=='A')?'greenbuttonelementsNew':'redbuttonelementsNew'?>">
                        <?=($row['status']=='A')?'Active':'Inactive'?>
                        </a> </td>
                      <td align="center"><a href="category_key.php?show=edit&id=<?=$row['id']?>&pId=<?=$parentId?>&secpid=<?=$secparent?>"><img src="images/edit.gif" title="Edit" width="16" height="16" border="0" /></a><a href="category_process_key.php?act=del&pageno=<?=($_REQUEST['pageno']!="")?$_REQUEST['pageno']:'0'?>&id=<?=$row['id']?>&secpid=<?=($_REQUEST['secpid']!="")?$_REQUEST['secpid']:'0'?>&pId=<?=($_REQUEST['pId']!="")?$_REQUEST['pId']:'0'?>"><img src="images/drop.gif" title="Delete" width="16" height="16" border="0" onClick="return confirm('Do you really want to delete this record');" /></a></td>
                    </tr>
                    <? }
...
?>
                ...
                <div class="bottomsecc">
					...
					<div class="clr"></div>
					<a class="brownbttn" href="category_key.php?show=add&pageno=<?=($_REQUEST['pageno']!="")?$_REQUEST['pageno']:'0'?>&secpid=<?=($_REQUEST['secpid']!="")?$_REQUEST['secpid']:'0'?>&pId=<?=($_REQUEST['pId']!="")?$_REQUEST['pId']:'0'?>">Add New Category</a>
				</div>
              </form>
              <? }
	  ...
	  if($show=='add') { ?>
              <form name="frmadd" id="frm2" method="post" action="category_process_key.php"  onsubmit="return add_typ_value()">
                <p>
                ...
                <table width="98%" align="center" cellPadding=6 cellSpacing=1 class="tborder_new">
                  <thead>
                    <tr>
                      <td colspan="5" align="left" class="style2">Add  Key Category Section </td>
                    </tr>
                  </thead>
                  <tbody>
                    <? if($_REQUEST['m']){ ?>
				...
				$sql1="SELECT * FROM ".$cfg['DB_KEYWORD_CATEGORY']." WHERE  `cat_parent_id` = '0' AND `status`='A' AND `siteId`= '".$cfg['SESSION_SITE']."'  ";
				...
				?>
                    ...
                    <tr>
						<td colspan="4" align="center">
							<a class="brownbttn" href="category_key.php?pageno=<?=($_REQUEST['pageno']!="")?$_REQUEST['pageno']:'0'?>&secpid=<?=($_REQUEST['secpid']!="")?$_REQUEST['secpid']:'0'?>&pId=<?=($_REQUEST['pId']!="")?$_REQUEST['pId']:'0'?>">&lt;&lt;back</a>
          ...
          if($show=='edit'){
            ...
            $sql="SELECT * FROM ". $cfg['DB_KEYWORD_CATEGORY']." WHERE  `id` =".$_REQUEST['id']."";
	  ...
	  ?>
              <form name="frmedit"  id="frm3" method="post" action="category_process_key.php" onsubmit="return edit_typ_value()">
                <p>
				...
				$sql1="SELECT * FROM ".$cfg['DB_KEYWORD_CATEGORY']." WHERE  `id` = '".$_REQUEST['id']."' AND `status`='A' AND `siteId`= '".$cfg['SESSION_SITE']."'";
				...
				?>
                          <option value="0">Parent Category</option>
                          <?php	$sql4="SELECT * FROM ".$cfg['DB_KEYWORD_CATEGORY']." WHERE `cat_parent_id`='0'";
				...
				?>
					...
					echo $sql2="select * from ".$cfg['DB_KEYWORD_CATEGORY']." where `cat_parent_id`='0'";
				...
				while($row2=$heart->sql_fetchrow($res2)){
			 	?>
                    ...
                    <tr class="row1">
                      ...
                      <td><div id="sec_parent">
                          <select name="secpid" id="secpid" class="forminputelement" onchange="category_availability();">
                            <?php
							if($row1['cat_parent_id']==0){
								$sql3="SELECT * FROM ".$cfg['DB_KEYWORD_CATEGORY']." WHERE  `cat_parent_id` = '".$_REQUEST['id']."' ";
							}
							else{
								$sql3="SELECT * FROM ".$cfg['DB_KEYWORD_CATEGORY']." WHERE  `cat_parent_id` = '".$_REQUEST['pId']."' ";
							}
                    ...
                    <tr>
						<td colspan="4" align="center">
							<a class="brownbttn" href="category_key.php?pageno=<?=($_REQUEST['pageno']!="")?$_REQUEST['pageno']:'0'?>">&lt;&lt;back</a>
							<input type="submit" name="Save" id="Save" value="Save" class="loginbttn">
                        </td>
                    </tr>
                  </tbody>
