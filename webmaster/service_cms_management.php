...
<td vAlign=top align="center" width="99%"><!-- Start Body Here -->
  <table width="100%" border="0" align="center" cellpadding="2" cellspacing="0">
	  ...
	  <? //show all record
	   ...
	   ?>
	    ...
	    <table width="98%" align="center" cellpadding="6" cellspacing="1" class="tborder_new">
          <thead>
            <tr>
              <td colspan="11" align="left">&nbsp;<span class="style2">Manage Service Mega-Menu CMS</span> </td>
            </tr>
          </thead>
          <tbody>
            <?php if($_REQUEST['m']){ ?>
            ...
            <tr class="headercontent">
				 ...
				 <td width="20%" align="center" colspan="3" class="leftBarText_new1">Service Mega Menu Name</td>
	             <td width="30%" align="center" colspan="3" class="leftBarText_new1">Description</td>
				 <td width="10%" align="center" class="leftBarText_new1">Status</td> 
				 <td width="10%" align="center" class="leftBarText_new1">Action</td>
            </tr>		  
			
		<?
		 	$sql    = "SELECT * FROM ".$cfg['DB_MEGA_MENU_CMS']." WHERE 
		...
		?>
            <tr class="<?=($i%2==0)?'row1':'row2'?>">
				...
				<td align="center"><input  name="checkvalue" id="checkvalue"  value="<?=$row['id']?>" type="checkbox" />	</td>
              	...
              	<td align="center" colspan="3" >&nbsp;<?=$row['subject'];?></td>
				<td align="center" colspan="3" >&nbsp;<?=$row['content'];?></td>
				<td align="center">
				  <a href="service-process.php?act=<?=($row['status']=='A')?'Inactive_menu':'Active_menu'?>&pageno=<?=($_REQUEST['pageno']!="")?$_REQUEST['pageno']:'0'?>&id=<?=$row['id']?>" class="<?=($row['status']=='A')?'greenbuttonelementsNew':'redbuttonelementsNew'?>"><?=($row['status']=='A')?'Active':'Inactive'?></a>
				  </td>
				  <td align="center">
					  <a href="service-process.php?act=edit_menu_cms&pageno=<?=($_REQUEST['pageno']!="")?$_REQUEST['pageno']:'0'?>&id=<?=$row['id']?>"><img src="images/edit.gif" title="Edit" width="16" height="16" border="0" /></a>
				  </td>
	...
	?>
		<form name="frmadd" id="frm2" method="post" action="service-process.php" onsubmit="return add_typ_value()" ><p>
          ...
          <table width="98%" align="center" cellPadding=6 cellSpacing=1 class="tborder_new"> 
            <thead> 
              <tr> 
                <td colspan="5" align="left" class="style2">&nbsp;Add Tech-support Info</td> 
              </tr> 
            </thead> 
            <tbody> 
              <? if($_REQUEST['m']){ ?>
	...
	// show edit window
	    ...
	    if($show=='edit')
		{
			$sql="SELECT * FROM ". $cfg['DB_MEGA_MENU_CMS'] ." 
	  ...
	  <form name="frmedit"  id="frm3" method="post" action="service-process.php" onsubmit="return edit_typ_value()">
          <p>
            <input type="hidden" name="act" value="update_mega_menu" />            
          ...
          <table width="98%" align="center" cellPadding=6 cellSpacing=1 class="tborder_new"> 
            <thead> 
              <tr> 
                <td colspan="5" align="left" class="style2">&nbsp;Edit <?php echo $row['subject'] ?> Section </td> 
              </tr> 
            </thead> 
            <tbody> 
              <? if($_REQUEST['m']){ ?>
			  ...
			  <? } ?>
				...
				<tr class="row2"> 
	                ...
	                <td  width="70%" colspan="4" class="leftBarText" align="left" valign="top">
	                	<textarea name="content" id="description"><?php echo $row['content'];?></textarea>
	                	<script>
		   ...
		   <td align="center" colspan="2">
				<a class="brownbttn" href="service_cms_management.php?pageno=<?=($_REQUEST['pageno']!="")?$_REQUEST['pageno']:'0'?>">&lt;&lt;back</a>
				<input type="submit" name="Save" id="Save" value="Save" class="loginbttn">
		   </td> 
