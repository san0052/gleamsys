...
page_header($cfg['ADMIN_TITLE']." - Shop Configuration Management");
...
<td vAlign=top align="center" width="99%"><!-- Start Body Here -->
	  ...
	  <? //show all record
	   if($_REQUEST['show']=='')
	   {
	  		/* $returnCatId=($_REQUEST['catId']=="")?'0':$_REQUEST['catId'];*/
	  		 ?>
	  		 <form name="frm1" id="frm1" action="shop_configuration_process.php" method="post">
	    	...
	    	<table width="98%" align="center" cellpadding="6" cellspacing="1" class="tborder_new">
        	  <thead>
          	  <tr>
             	 <td colspan="10" align="left">&nbsp;<span class="style2">Shop Configuration</span> </td>
              </tr>
          	</thead>
         	 <tbody>
            	<? if($_REQUEST['m']){ ?>
            ...
            <tr class="headercontent">
			  ...
			  <td width="16%" align="center" class="leftBarText_new1">Db No </td>
			  <td width="40%" align="center" class="leftBarText_new1">Shipping Cost</td>
			   <td width="23%" align="center" class="leftBarText_new1">Free Shipping Limit</td>
			  <td width="40%" align="center" class="leftBarText_new1">Order Id</td>			
			  <td width="25%" align="center" class="leftBarText_new1">Action</td>
            </tr>		  
		<? 
		
			$sql="SELECT * FROM ".$cfg['DB_SHOP_CONFIG']." where `siteId`='".$cfg['SESSION_SITE']."'";
			$res=$heart->sql_query($sql);
			...
			?>
        	    <tr class="<?=($i%2==0)?'row1':'row2'?>">
				  ...
				  <td align="center"><?=$row['sc_id']?></td>
        
				  <td align="center" class="leftBarText"><?=$row['sc_shipping_cost'] ?> </td>
				   <td align="center" class="leftBarText"><?=$row['sc_free_shipping_limit'] ?> </td>
			   <td align="center" class="leftBarText"><?=$row['order_id_pattern'] ?> </td>
	             
			  
				<td align="center">
					
					<a href="shop_configuration_process.php?act=edit&pageno=<?=($_REQUEST['pageno']!="")?$_REQUEST['pageno']:'0'?>&id=<?=$row['sc_id']?>"><img src="images/edit.gif" title="Edit" width="16" height="16" border="0" /></a>			  	</td>
            </tr>
			<? }
			}
			else
		...
		<? }
          ...
          if($show=='edit')
		  {
            $sql="SELECT * FROM ". $cfg['DB_SHOP_CONFIG']." where `siteId`='".$cfg['SESSION_SITE']."' and `sc_id` = ".$_REQUEST['id']."";
			$res=$heart->sql_query($sql);
			$row=$heart->sql_fetchrow($res);
	  ?>
	  <form name="frmedit"  id="frm3" method="post" action="shop_configuration_process.php" onsubmit="return edit_typ_value()">
          <p>
          ...
          <table width="98%" align="center" cellPadding=6 cellSpacing=1 class="tborder_new"> 
            ...
            <tbody> 
			  ...
			  <? } ?>
				...
				<tr class="row1"> 
                <td width="30%" align="left" class="leftBarText"><span class="leftBarText_new">Change Shipping Cost</span></td> 
                <td width="70%" colspan="4" align="left">
				<input name="sc_shipping_cost" type="text" class="forminputelement" id="sc_shipping_cost" value="<?=$row['sc_shipping_cost'] ?>"/>&nbsp;&nbsp;</td>
 				 </tr> 
				 
				 <tr class="row1"> 
                <td width="30%" align="left" class="leftBarText"><span class="leftBarText_new">Change Shipping Limt</span></td> 
                <td width="70%" colspan="4" align="left">
				<input name="sc_free_shipping_limit" type="text" class="forminputelement" id="sc_free_shipping_limit" value="<?=$row['sc_free_shipping_limit'] ?>"/>&nbsp;&nbsp;</td>
 				 </tr>
				 
				 <tr class="row2"> 
                <td width="30%" align="left" class="leftBarText"><span class="leftBarText_new">Change Order Id Pattern</span></td> 
                <td width="70%" colspan="4" align="left">
				<input name="order_id_pattern" type="text" class="forminputelement" id="order_id_pattern" value="<?=$row['order_id_pattern'] ?>"/>&nbsp;&nbsp;</td>
 				 </tr> 
  ...
  <tr> 
   <td align="right"><a class="brownbttn" href="shop_configuration.php?pageno=<?=($_REQUEST['pageno']!="")?$_REQUEST['pageno']:'0'?>">&lt;&lt;back</a></td> 
