	...
	*/
	...
	function redirect($url,$message='',$time='1')
	{
		...
		if (!headers_sent()) {
			...
			$re_err.="<meta http-equiv='Refresh' content='".$time.";url=".$newURl."'>\n";
			/*$re_err.="<table style='display:none' width=\"50%\" border=\"1\" align=\"center\" cellpadding=\"4\" cellspacing=\"0\">\n";
			$re_err.="\t<tr>\n";
			$re_err.="\t\t<td align=\"left\" style=\"background-color: #006699; color: #ffffff;\"><font face=\"Verdana\" size=\"2\"><B>".$message."</B></font></td>\n";
			$re_err.="\t</tr>\n";
			$re_err.="\t<tr>\n";
			$re_err.="\t\t<td align=\"left\" style=\"background-color: #D5D5D5; color: #000;\">";
			$re_err.="<font face=\"Verdana\" size=\"2\">If page does not refresh automatically please click on this link </font><BR>";
			$re_err.="<a href='".$newURl."'><font face=\"Verdana\" size=\"2\">".$newURl."</font></a></td>\n";
			$re_err.="\t</tr>\n";
			$re_err.="</table>\n";
			$re_err.="<center><h1>Processing....</h1></center>";*/
			$re_err.="</body>";
			$re_err.="</html>";
			die($re_err);
		}
	...
	*/
	...
	function login($uname,$pass,$trackIp=false){
		...
		if ($this->sql_numrows($user_check_result)!=0){
			...
			if($trackIp){
				$insert_ip="INSERT INTO ".$GLOBALS['ddc']['DB_MEMBERLOGS']." ( `sid`, `current_ip` ,`member_id` , `lastlogin` ) ";
				$insert_ip.="VALUES ('".session_id()."','".$GLOBALS['ddc']['IP']."', '".$_SESSION['admin_login_uid']."','".$GLOBALS['ddc']['TODAY']."')";
				$this->sql_query($insert_ip);
			}
			// END OF IP ADDRESS TRACKING SYSTEM
