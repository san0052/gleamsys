...
page_header($cfg['ADMIN_TITLE']." - Notes Management");
...
<td vAlign=top align="center" width="99%"><!-- Start Body Here -->
  <table width="100%" border="0" align="center" cellpadding="2" cellspacing="0">
	   ...
	   if($_REQUEST['show']=='')
	   	{
			...
			?>
				<form name="frm1" id="frm1" action="notes_process.php" method="post">
				...
				<thead>
					<tr>
					<td colspan="11" align="left">&nbsp;<span class="style2">Special Message</span> </td>
					</tr>
				</thead>
				<tbody>
					<? if($_REQUEST['m']){ ?>
					...
					<tr class="headercontent">
						...
						<td width="6%" align="center" class="leftBarText_new1">Title</td>
						
						<td width="17%" align="center"  colspan="3" class="leftBarText_new1">Message</td>
					
						<td width="23%" align="center"   colspan="3" class="leftBarText_new1">Created Date</td>
				...
				<?
					$sql="SELECT * FROM ".$cfg['DB_SPECIAL_MESSAGE']." WHERE `status`!= 'D' ";
					$res=$heart->sql_query($sql);
				...
				?>
					<tr class="<?=($i%2==0)?'row1':'row2'?>">
						...
						<td align="center"><input  name="checkvalue" id="checkvalue"  value="<?=$row['n_id']?>" type="checkbox" />	</td>
					
						<td align="center"><?=$i+$offset?></td>
						<td align="center"><?=$row['title']?></td>
					
						<td align="center" colspan="3" >&nbsp;<?=$row['message']?></td>
						<td align="center" colspan="3" >&nbsp;<?=getdataformat($row['created_date'])?></td>		 	  
					
						<td align="center">
					
						<a href="manage_special_message_process.php?act=<?=($row['status']=='A')?'Inactive':'Active'?>&pageno=<?=($_REQUEST['pageno']!="")?$_REQUEST['pageno']:'0'?>&id=<?=$row['id']?>" class="<?=($row['status']=='A')?'greenbuttonelementsNew':'redbuttonelementsNew'?>"><?=($row['status']=='A')?'Active':'Inactive'?></a>
						
						</td>
					
					<td align="center">
						<a href="manage-specialmessage.php?show=view&pageno=<?=($_REQUEST['pageno']!="")?$_REQUEST['pageno']:'0'?>&id=<?=$row['id']?>"><img src="images/view.gif" title="View" width="16" height="16" border="0" /></a>
					
						<a href="manage-specialmessage.php?show=edit&pageno=<?=($_REQUEST['pageno']!="")?$_REQUEST['pageno']:'0'?>&id=<?=$row['id']?>"><img src="images/edit.gif" title="Edit" width="16" height="16" border="0" /></a>
					
						<a href="manage_special_message_process.php?act=del&pageno=<?=($_REQUEST['pageno']!="")?$_REQUEST['pageno']:'0'?>&id=<?=$row['id']?>"><img src="images/drop.gif" title="Delete" width="16" height="16" border="0" onClick="return confirm('Do you really want to delete this record ?');" /></a>
					...
					<div class="bottomsecc">		
						<a class="brownbttn" href="manage-specialmessage.php?show=add&pageno=<?=($_REQUEST['pageno']!="")?$_REQUEST['pageno']:'0'?>">Add Message</a>
					</div>
	...
	{ 
				...
				?>
					<form name="frmadd" id="frm2" method="post" action="manage_special_message_process.php" onsubmit="return add_typ_value()" ><p>
					...
					<table width="98%" align="center" cellPadding=6 cellSpacing=1 class="tborder_new"> 
						<thead> 
						<tr> 
							<td colspan="5" align="left" class="style2">&nbsp;Add Message</td> 
						</tr> 
						</thead> 
						<tbody> 
						<? if($_REQUEST['m']){ ?>
						...
						<tr class="row1"> 
							...
							<td width="70%" colspan="4" align="left"><input name="title" type="text" class="forminputelement" id="title" value=""/>&nbsp;&nbsp;</td>
						</tr>	
						
						<tr class="row2"> 
							<td width="30%" align="left" class="leftBarText"><span class="leftBarText_new">Message </span><span class="redstar">*</span></td> 
							<td  class="leftBarText" align="left" valign="top">&nbsp;</td>
						</tr>
							
							<tr class="row1">
							<td colspan="2" align="center">
								<textarea name="des_name" id="des_name" class="forminputelement textareawid" rows="4"></textarea>	
							</td>
						</tr>
							
						<tr> 
							<td align="center" colspan="2">
								<a class="brownbttn" href="notes.php?pageno=<?=($_REQUEST['pageno']!="")?$_REQUEST['pageno']:'0'?>">&lt;&lt;back</a>
 ...
 <? }
		...
		{ 
						...
						$sql="SELECT * FROM " .$cfg['DB_SPECIAL_MESSAGE']. "WHERE `status`= 'A'";
						$res=$heart->sql_query($sql);
				...
				<table width="98%" align="center" cellPadding=6 cellSpacing=1 class="tborder_new"> 
					<thead> 
					<tr> 
						<td colspan="5" align="left" class="style2">&nbsp;Details Message</td> 
						<td colspan="5" align="right" class="style2">
					<a style="text-decoration:none; color:#FFFFFF;" href="disclaimer_process.php?act=edit&pageno=<?=($_REQUEST['pageno']!="")?$_REQUEST['pageno']:'0'?>&id=<?=$row['n_id']?>"><strong>[Edit]</strong></a></td>
					</tr> 
					</thead> 
					<tbody> 
					<?php /*?> <? if($_REQUEST['m']){ ?>
					...
					<tr class="row1"> 
						...
						<td width="70%"  align="left"><?=stripslashes($row['title'])?></td>
					</tr>
					<tr class="row2"> 
						<td width="30%" align="left" class="leftBarText"><span class="leftBarText_new">Message</span></td>
						<td width="70%"  align="left"><?=$row['message']?></td>
					</tr>
					<tr class="row1"> 
						<td width="30%" align="left" class="leftBarText"><span class="leftBarText_new">Date</span></td> 
						<td width="70%"  align="left"><?=getdataformat($row['created_date'])?></td>
					</tr>
					
					<tr class="row2"> 
						<td width="30%" align="left" class="leftBarText"><span class="leftBarText_new">Status</span></td> 
						<td width="70%"  align="left"><?=($row['status']=='A')?'Active':'Inactive'?></td>
					</tr>
					
					<tr>
						<td width="100%" align="center"><a class="brownbttn" href="manage-specialmessage.php?pageno=<?=($_REQUEST['pageno']!="")?$_REQUEST['pageno']:'0'?>">&lt;&lt;back</a></td >
					</tr> 
				</tbody> 
				</table> 
	...
	// show edit window
	         ...
	         if($show=='edit')
			 {
						...
						$sql="SELECT * FROM ". $cfg['DB_SPECIAL_MESSAGE'] ." where `status` = 'A' AND `id` = '".$id."' "; 
						$res=$heart->sql_query($sql);
						$row=$heart->sql_fetchrow($res);
					?>
					<form name="frmedit"  id="frm3" method="post" action="manage_special_message_process.php" onsubmit="return edit_typ_value()">
						<p>
						...
						<tr class="row2"> 
								<td width="30%" align="left" class="leftBarText"><span class="leftBarText_new">Change Message</span><span class="redstar">*</span></td> 
								<td  class="leftBarText" align="left" valign="top">&nbsp;</td>
								
						</tr>
						<tr class="row1">
								<td colspan="2" align="center">
									<textarea name="des_name" id="des_name" class="forminputelement textareawid" rows="4"><?=stripslashes($row['message'])?></textarea>			
								</td>
						</tr>
						<td align="center" colspan="2">
									<a class="brownbttn" href="manage-specialmessage.php?pageno=<?=($_REQUEST['pageno']!="")?$_REQUEST['pageno']:'0'?>">&lt;&lt;back</a>
									<input type="submit" name="Save" id="Save" value="Save" class="loginbttn">
						</td> 
