...
class EasyGoogleMap {
	...
	/**
	 * @desc: 	Set Overlay
	 * @param: 	boolean [options('TRUE','FALSE')]
	 * @access:	public
	 * @return: void
	 */
	function SetOverlay($overlay=true) {
	...
	function InitJs() {
		...
		if ($is_error) { 
			...
			$ret .= "	var map = new GMap2(document.getElementById('TheGoogleMap'), {size:new GSize(".$this->mMapWidth.",  ".$this->mMapHeight.")}); \n";
			...
			# handle map type (map/satelite/hybrid)
			...
			for ($i=$cnt_add-1; $i>=0; $i--) {
				...
				$ret .= "		if(point) {\n";
				$ret .= "		  points[$i] = point; \n";	
				$ret .= "		  map.setCenter(point, {$this->mMapZoom}, G_HYBRID_MAP);\n";
//								$ret .= "		  map.setCenter(point, {$this->mMapZoom}, G_SATELLITE_MAP);\n";
	 ...
	 */
	function MapHolder() {
		return "<div id='TheGoogleMap' style=\"width:".$this->mMapWidth."px; height:".$this->mMapHeight."px; \" align='center' ></div>";
	} # end function
