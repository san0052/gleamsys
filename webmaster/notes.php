  ...
  <table width="100%" border="0" align="center" cellpadding="2" cellspacing="0">
	  ...
	  <? //show all record
	   ...
	   ?>
	    ...
	    <table width="98%" align="center" cellpadding="6" cellspacing="1" class="tborder_new">
          <thead>
            <tr>
              <td colspan="11" align="left">&nbsp;<span class="style2">Details of Notes</span> </td>
            </tr>
          </thead>
          <tbody>
            <? if($_REQUEST['m']){ ?>
		...
		<?
			$sql="SELECT * FROM ".$cfg['DB_NOTES']." WHERE `siteId`= '".$cfg['SESSION_SITE']."' ";
			$res=$heart->sql_query($sql);
		...
		?>
            <tr class="<?=($i%2==0)?'row1':'row2'?>">
				...
				<td align="center"><?=$row['n_id']?></td>
			  
              	<td align="center" colspan="3" >&nbsp;<?=$row['title']?></td>
				<td align="center" colspan="3" >&nbsp;<?=getdataformat($row['n_date'])?></td>		 	  
			  
              	<td align="center">
			  
				  <a href="notes_process.php?act=<?=($row['status']=='A')?'Inactive':'Active'?>&pageno=<?=($_REQUEST['pageno']!="")?$_REQUEST['pageno']:'0'?>&id=<?=$row['n_id']?>" class="<?=($row['status']=='A')?'greenbuttonelementsNew':'redbuttonelementsNew'?>"><?=($row['status']=='A')?'Active':'Inactive'?></a>
				  
				  </td>
			  
			  <td align="center">
			  	  <a href="notes.php?show=view&pageno=<?=($_REQUEST['pageno']!="")?$_REQUEST['pageno']:'0'?>&id=<?=$row['n_id']?>"><img src="images/view.gif" title="View" width="16" height="16" border="0" /></a>
			  
				  <a href="notes_process.php?act=edit&pageno=<?=($_REQUEST['pageno']!="")?$_REQUEST['pageno']:'0'?>&id=<?=$row['n_id']?>"><img src="images/edit.gif" title="Edit" width="16" height="16" border="0" /></a>
			  
				  <a href="notes_process.php?act=del&pageno=<?=($_REQUEST['pageno']!="")?$_REQUEST['pageno']:'0'?>&id=<?=$row['n_id']?>"><img src="images/drop.gif" title="Delete" width="16" height="16" border="0" onClick="return confirm('Do you really want to delete this record ?');" /></a>
		...
		<div class="bottomsecc">		
			<a class="brownbttn" href="notes.php?show=add&pageno=<?=($_REQUEST['pageno']!="")?$_REQUEST['pageno']:'0'?>">Add New Notes</a>
		</div>
	...
	<tr><td colspan="3" align="right"></td></tr>
		...
		{ 
			 	...
			 	$sql="SELECT * FROM " .$cfg['DB_NOTES']. "WHERE `n_id`=".$_REQUEST['id']." AND `siteId`= '".$cfg['SESSION_SITE']."' ";
				$res=$heart->sql_query($sql);
		...
		?>
          ...
          <table width="98%" align="center" cellPadding=6 cellSpacing=1 class="tborder_new"> 
            <thead> 
              <tr> 
                <td colspan="5" align="left" class="style2">&nbsp;Details of Notes</td> 
			...
			<tr class="row2"> 
                ...
                <td width="70%" colspan="9" align="left"><?=$row['n_date']?></td>
			</tr>
			<tr class="row1"> 
			...
			<tr>
				<td width="100%" align="center"><a class="brownbttn" href="notes.php?pageno=<?=($_REQUEST['pageno']!="")?$_REQUEST['pageno']:'0'?>">&lt;&lt;back</a></td >
			</tr> 
          </tbody> 
          </table> 
	...
	?>
		<form name="frmadd" id="frm2" method="post" action="notes_process.php" onsubmit="return add_typ_value()" ><p>
		    <input type="hidden" name="pageno" value="<?=$_REQUEST['pageno']?>" />
          ...
          <table width="98%" align="center" cellPadding=6 cellSpacing=1 class="tborder_new"> 
            <thead> 
              <tr> 
                <td colspan="5" align="left" class="style2">&nbsp;Add Notes</td> 
              </tr> 
            </thead> 
            <tbody> 
              <? if($_REQUEST['m']){ ?>
			...
			<tr class="row1"> 
                ...
                <td width="70%" colspan="4" align="left"><input name="title" type="text" class="forminputelement" id="title" value="<?=($_SESSION['title']!='')?stripslashes($_SESSION['title']):''?>"/>&nbsp;&nbsp;</td>
			</tr>	
	...
	// show edit window
	         ...
	         if($show=='edit')
			 {
				...
				$row=$heart->sql_fetchrow($res);
	  ?>
	  <form name="frmedit"  id="frm3" method="post" action="notes_process.php" onsubmit="return edit_typ_value()">
          <p>
          ...
          <table width="98%" align="center" cellPadding=6 cellSpacing=1 class="tborder_new"> 
			...
			<input name="title" type="text" class="forminputelement" id="title" value="<?=($_SESSION['title']!='')?stripslashes($_SESSION['title']):stripslashes($row['title'])?>"/>
		...
		<tr class="row2"> 
 		...
 		<tr class="row1">
                 <td colspan="2" align="center">
					<textarea name="des_name" id="des_name" class="forminputelement textareawid" rows="4"><?=stripslashes($row['description'])?></textarea>			
				 </td>
           </tr>
		   <td align="center" colspan="2">
					<a class="brownbttn" href="notes.php?pageno=<?=($_REQUEST['pageno']!="")?$_REQUEST['pageno']:'0'?>">&lt;&lt;back</a>
